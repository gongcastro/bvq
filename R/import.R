...
#'
#' @md
collect_survey <- function(version, participants = bvq_participants(), ...) {
  ...
  participants_tmp <- participants_tmp[,
    colnames(participants_tmp) != "version"
  ]
...
download_surveys <- function(surveys, verbose = TRUE, ...) {
  ...
  for (i in seq_along(surveys)) {
    ...
    if (verbose && interactive()) cli::cli_progress_update()
  }
...
#' @param type character string indicating whether to process the "vocabulary" or the "grammar" questionnaire. Defaults to "vocabulary".
#'
...
process_survey <- function(
  ...
  # check `type` argument
  if (!(type %in% c("vocabulary", "grammar"))) {
    cli::cli_abort(
      "{.code type} must be one of {.val vocabulary} or {.val grammar}"
    )
  }
  # subset columns
  words_cat <- select(
  ...
  processed_tmp <- raw_tmp %>%
    ...
    dplyr::filter(
      response_id %in% participants_tmp$response_id,
  ...
  if (type == "grammar") {
    processed <- processed_tmp %>%
      select(
        child_id,
        one_of(items_to_keep),
        starts_with("cat_grammar"),
        starts_with("spa_grammar")
      ) |>
      mutate(across(matches("grammar_"), as.character)) |>
  ...
  processed <- processed %>%
    pivot_longer(
      ...
      names_to = "item",
      values_to = "response",
      names_transform = \(x) gsub("grammar_", "", x)
    ) %>%
...
fix_logs_df <- function(raw, participants_tmp) {
  ...
  logs$response_id <- fix_response_id(if_else(
    logs$response_id == "",
    NA_character_,
    gsub("BL", "", logs$response_id)
  ))
  logs <- logs[
    !is.na(logs$response_id) & !is.na(logs$ended) & !is.na(logs$session),
  ]
