...
verbose = TRUE) {
    ...
    participants_tmp <- get("participants", parent.frame()) %>% 
        >>>>>>> 511b8776961098a2f3f994c9189a8bf5fb7f5292
    select(-version)
    ...
    raw[[1]] <- raw[[1]] %>%
        ...
        rename(code = bl_code) %>%
        <<<<<<< HEAD
    mutate(# fix codes known to be wrong
    ...
    verbose = TRUE) {
        ...
        processed <- raw %>%
            <<<<<<< HEAD
        map(select, -one_of(c(
            "created", "modified", "ended", "expired"
        ))) %>%
        ...
        left_join(
            select(
                raw$bilexicon_short_06_words_cat,
                session,
                created_cat = created,
                ended_cat = ended
            ),
            ...
            left_join(
                select(
                    raw$bilexicon_short_06_words_spa,
                    session,
                    created_spa = created,
                    ended_spa = ended
                ),
        ...
        filter(code %in% participants_tmp$code) %>%
            ...
            mutate(
                ...
                <<<<<<< HEAD
                postcode = as.integer(ifelse(
                    postcode %in% "", NA_character_, postcode
                )),
                =======
                    postcode = na_if(as.character(postcode), ""),
                >>>>>>> 511b8776961098a2f3f994c9189a8bf5fb7f5292
