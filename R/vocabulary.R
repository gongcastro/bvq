...
#'   variables provided in \code{.by}. This variables can refer to item
#'   properties (see \code{pool}, e.g., "category") or to participant properties
#'   (see \code{bvq_logs()}, e.g., "dominance").
#' @param scale A character vector that takes the value "count" and/or "prop".
#'   If "count" (default) vocabulary metrics are reported as counts (number of
#'   words). If "prop", vocabulary metrics are calculated as proportions?
#' @param ... Extra arguments that will be passed to the \code{bvq_responses}
#'   function.
#' @return A dataset (actually, a [tibble::tibble] with each participant's
...
#' }
#'   The specific subset of columns returned by `bvq_vocabulary` depends on
