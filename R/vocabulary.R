...
#' @param responses Responses data frame, as generated by \code{bvq_responses}. If NULL (default), \code{bvq_responses} is run.
#' @param by A character vector that takes the name of the variable(s) to group data into. Vocabulary metrics will be calculated by aggregating responses within the groups that result from the combination of crossing of the variables provided in \code{by}. This variables can refer to item properties (see \code{pool}, e.g., "category") or to participant properties (see \code{bvq_logs()}, e.g., "dominance").
...
#' @param ... Extra arguments that will be passed to the \code{bvq_responses} function.
...
#' }
#' The specific subset of columns returned by \code{bvq_vocabulary} depends on the arguments provided.
...
bvq_vocabulary <- function(participants = NULL,
    ...
    suppressMessages({
        ...
        vocab_total_dominance <- vocab_base %>%
            ...
            pivot_wider(names_from = item_dominance, values_from = c(n_total, matches("vocab"))) %>%
            clean_names() %>%
        ...
        vocab_te <- vocab_base %>%
            ...
            mutate(
                vocab_prop_te = vocab_count_te/n_total,
                vocab_count_te = as.integer(vocab_count_te)
            ) %>%
            select(id, time, age, type, starts_with("vocab_count"), starts_with("vocab_prop"), any_of(by))
        
        # merge all datasets
        vocab <- list(vocab_total, vocab_total_dominance, vocab_conceptual, vocab_te) %>% 
            reduce(left_join, by = c("id", "time", "age", "type", by)) %>%
