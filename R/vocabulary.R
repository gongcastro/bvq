...
bvq_vocabulary <- function(participants,
    ...
    # check if all extra ... are valid colnames
    possible_colnames <- unlist(map(list(responses, logs, pool), colnames))
    check_arg_by(dots_vctr, .cols = possible_colnames)
    ...
    vocab_base <- responses %>%
        ...
        select(id, time, dominance, item_dominance, type, 
    ...
    vocab_total_dominance <- vocab_base %>%
        summarise(count = sum(response, na.rm = TRUE),
                  ...
                  .by = c(id, time, type, item_dominance, ...)) %>%
        ...
        clean_names() %>%
        select(id, time, type, starts_with("l1_"),  starts_with("l2_"), 
               -ends_with("n_total"), any_of(dots_vctr))
    ...
    vocab_te <- vocab_by_te_base %>%
        ...
        select(id, time, type, te_count, te_prop, any_of(dots_vctr))
