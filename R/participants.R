...
#'   participants that have participated or are candidates to participate in any
#'   of the versions of BVQ
...
#'   versions of MultiLex. Each row corresponds to a questionnaire response and
#'   each column represents a variable. The output includes the following
#'   variables:
#'  \describe{
#'      \item{id}{a character string indicating a participant's identifier. This value is always the same for each participant, so that different responses from the same participant share the same `id`.}
...
#'      \item{date_birth}{a date value (see lubridate package) in `yyyy/mm/dd` format indicating participants birth date.}
#'      \item{age_now}{a numeric value indicating the number of months elapsed since participants' birth date until the present day, as indicated by [lubridate::now()]}
...
#'   \item{randomisation}{a character string indicating the specific list of
#'   items a participant was assigned to. Only applies in the case of short
#'   versions of multilex, such as BL-Short, BL-Short-2 or BL-Lockdown, where
#'   the list of items was partinioned into several versions.}
#'   \item{date_test}{a date value (see lubridate package) in `yyyy/mm/dd`
#'   format indicating the date in which the participant was tested in the
#'   associated study, if any} \item{date_sent}{a date value (see lubridate
#'   package) in `yyyy/mm/dd}` format indicating the date in which the
#'   participant was sent the questionnaire} \item{call}{a character string
#'   indicating the status of the participant's response: *successful*
...
#'   two weeks since they were sent the questionnaire).} \item{comments}{a
#'   character string indicating useful information for database management} }
